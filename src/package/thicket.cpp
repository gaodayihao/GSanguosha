...
public:
    ...
    virtual bool trigger(TriggerEvent, Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(effect.card->inherits("SavageAssault")){
            ...
            room->setEmotion(player, QString("skill/%1").arg(avoid_skill));
            return true;
        }else
            return false;
    }
...
public:
    ...
    virtual bool trigger(TriggerEvent , Room* room, ServerPlayer *player, QVariant &data) const{
        ...
        if(use.card->inherits("SavageAssault") && use.from && use.from != player){
...
public:
    ...
    const Card *askForDoubleJink(ServerPlayer *player, ServerPlayer *from, const QString &reason) const{
        Room *room = player->getRoom();

        const Card *first_jink = NULL, *second_jink = NULL;
        first_jink = room->askForCard(player, "jink", QString("@%1-jink-1").arg(reason), QVariant(), CardUsed, from);
        if(first_jink)
            second_jink = room->askForCard(player, "jink", QString("@%1-jink-2").arg(reason), QVariant(), CardUsed, from);
