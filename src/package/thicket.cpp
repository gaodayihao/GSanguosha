...
public:
    ...
    virtual bool trigger(TriggerEvent event, ServerPlayer *player, QVariant &data) const{
        ...
        if(effect.card->inherits("SavageAssault")){
            ...
            player->getRoom()->setEmotion(player, QString("skill/%1").arg(avoid_skill));
            return true;
        }else
            return false;
    }
...
void LuanwuCard::onEffect(const CardEffectStruct &effect) const{
    ...
    if(!luanwu_targets.isEmpty() && (slash = room->askForCard(effect.to, "slash", "@luanwu-slash-Use", NULL, NonTrigger))){
