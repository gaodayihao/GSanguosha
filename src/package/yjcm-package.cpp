    ...
    virtual bool trigger(TriggerEvent , ServerPlayer *player, QVariant &data) const{
        ...
        if(effect.card->getTypeId() == Card::Trick){
            ...
            if((effect.from && effect.from->hasSkill(objectName()))){
                ...
                room->setEmotion(effect.from, QString("skill/%1").arg(objectName()));
                return true;
            }
            ...
            if(effect.to->hasSkill(objectName()) && effect.from){
                ...
                room->setEmotion(effect.to, QString("skill/%1").arg(objectName()));
                return true;
            }
        }
...
void XianzhenSlashCard::onUse(Room *room, const CardUseStruct &card_use) const{
    ...
    const Card *slash = room->askForCard(card_use.from, "slash", "@xianzhen-slash-Use", NULL, NonTrigger);
    if(slash){
        CardUseStruct use;
        use.card = slash;
...
void GanluCard::swapEquip(ServerPlayer *first, ServerPlayer *second) const{
    ...
    DummyCard *e1 = new DummyCard;
    DummyCard *e2 = new DummyCard;
    for (int i=0; i<4; i++){
        if (first->getEquip(i))
            e1->addSubcard(first->getEquip(i));
        if (second->getEquip(i))
            e2->addSubcard(second->getEquip(i));
    }

    room->ExchangeCards(first, second, e1, e2, Player::Equip, true);
