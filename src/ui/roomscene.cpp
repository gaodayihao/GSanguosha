      ...
      main_window(main_window),game_started(false)
{
    ...
    {
        prompt_box = new Window(tr("QSanguosha-GUNDAM"), QSize(480, 200));
...
QList<QPointF> RoomScene::getPhotoPositions() const{
    ...
    static const QPointF pos[] = {
        ...
        QPointF((85+cxw2*139)+(-six*cxw*80)+(seven*cxw*58)+(-nine*cxw*45), (-316+cxw2*22)+(-six*cxw*8)+(seven*cxw*30)+(cxw*30*st)), // 6:shuangxiong
...
void RoomScene::drawCards(const QList<const Card *> &cards){
    ...
    if(Self->hasFlag("SpecialDraw"))
        log_box->appendLog(
                "#SpecialDraw",
                Self->getGeneralName(),
                QStringList(),
                QString(),
                QString::number(cards.length())
                );
    else
    ...
    if(status != Client::NotActive){
        if(focused){
            ...
            if(focused->getPlayer()->getHp() <= 0 && focused->getPlayer()->isAlive()
                && focused->getPlayer()->getMaxHP()>0 && focused->getPlayer()->getPhase() == Player::NotActive)
                focused->setFrame(Photo::SOS);
            else if(focused->getPlayer()->getPhase()!=Player::NotActive && focused->getPlayer()->getPhase()!=Player::Finish)
                focused->setFrame(Photo::Playing);
            else
...
void RoomScene::speak(){
    if(chat_edit->text().trimmed() != ""){
        ...
        if(sets.at(0).toLower().trimmed() == "settextcolor"){
            ...
            Config.setValue("TextColor",sets.at(1).trimmed());
        }else if(sets.at(0).toLower().trimmed() == "settextb"){
            Config.TextB = sets.at(1).trimmed();
            Config.setValue("TextB",sets.at(1).trimmed());
        }else{
            if(Config.TextB=="0")
                ClientInstance->speakToServer("<font color="+Config.TextColor+">" + chat_edit->text()+ "</font>");
            else
                ClientInstance->speakToServer("<b><font color="+Config.TextColor+">" + chat_edit->text()+ "</font></b>");
        }
    }
    chat_edit->clear();
}
...
void RoomScene::createStateItem(){
    ...
    if(ServerInfo.EnableAI){
        ...
        if(circular){
            add_robot->setPos(-605, 275);
            fill_robots->setPos(-605, 220);
            ready_button->setPos(-605, 165);
        }
    }else
    {
        QRectF state_rect = state_item->boundingRect();
...
void RoomScene::onGameStart(){
    ...
    QString bgmusic_path = Config.value("BackgroundMusic", "audio/Professional2/system/background.ogg").toString();
...
//    if(Self->getRole() != "lord"){
    if(!Self->isOwner()){
        QMessageBox::warning(main_window, tr("Warning"), tr("Only the Owner can kick!"));
        return;
    }
    ...
    if(items.length() < 1){
        QMessageBox::warning(main_window, tr("Warning"), tr("Here is no player to kick"));
        return;
    }
